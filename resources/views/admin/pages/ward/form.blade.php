...
@php
    ...
    $dataType = config('configs.location.ward');
    $elements = [
        [
            'label' => html()
        ...
        [
            'label' => html()
                ->label(for: 'name', contents: 'Tên Quận / Huyện')
                ->attributes(['class' => $formLabelAttr]),
            'element' => html()
                ->select('district_id', $districtItems, @$item['district_id'])
                ->attributes([
                    'class' => $errors->first('district_id') ? $formInputAttr . ' is-invalid' : $formInputAttr,
                ]),
            'error' => $errors->first('district_id')
                ? sprintf('<div class="invalid-feedback">%s</div>', $errors->first('district_id'))
                : '',
        ],
        [
